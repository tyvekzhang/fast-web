...
def register_router(
    ...
    for controller_item in controller_dirs:
        ...
        for controller_file in controller_dir.glob(
            CONTROLLER_FILE_PATTERN_TEMPLATE.format(controller_flag)
        ):
            ...
            module_path = (
                f"{MODULE_BASE_PREFIX}{relative_path}.{module_name}".replace(
                    "/", MODULE_SEPARATOR
