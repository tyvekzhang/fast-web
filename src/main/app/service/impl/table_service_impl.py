...
class TableServiceImpl(BaseServiceImpl[GenTableMapper, TableModel], TableService):
    ...
    async def import_gen_table(self, table_import: TableImport):
        ...
        table_records: List[MetaTableModel] = await tableMapper.select_by_ids(
            ids=table_ids
        )
    ...
    async def generator_code(self, gen_table_id: int):
        ...
        # 通过字段的id查询子字段的信息
        gen_field_records: List[GenFieldDO] = (
    ...
    async def get_table_data(cls, *, id: int, current: int, pageSize: int):
        # 根据生成表关联的数据库表ID查询表信息
        table_do: MetaTableModel = await tableMapper.select_by_id(id=id)
    ...
    async def get_gen_table_detail(self, *, id: int) -> GenTableDetail:
        ...
        # 获取数据库表信息
        db_table: MetaTableModel = await tableMapper.select_by_id(id=table_id)
